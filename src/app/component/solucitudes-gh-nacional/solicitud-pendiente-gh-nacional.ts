...
@Component({
  ...
  templateUrl: './solicitud-pendiente-gh-nacional.html',
  styleUrls: ['./solicitud-pendiente-gh-nacional.css'],
})
export class SolicitudPendienteGhNacional implements OnInit, OnDestroy, AfterViewInit {
  ...
  private loadCatalogsAndData() {
    ...
    // Mapa temporal de regionales - reemplazar con servicio real
  ...
  private initRegionalMap() {
    // Mapa temporal - reemplazar con datos del servicio
  ...
  private mapToTiempoRegistrado(d: any): ITiempoRegistrado {
    return {
      ...
      regional: this.resolveRegional(d)
  ...
  private resolveRegional(d: any): string {
    return d.regionalNombre || d.nombreRegional || 'Sin Regional';
  }

  private parseDateTime(obj: any, dateKey: string, timeKey: string): Date | null {
    const dateVal = obj[dateKey] || obj[dateKey.toLowerCase()];
    const timeVal = obj[timeKey] || obj[timeKey.toLowerCase()];
